<?php if (!defined('THINK_PATH')) exit();?>
<!DOCTYPE html>
<html><head>
        <meta charset="UTF-8">
        <script>
            window._jHeadStart = ( new Date() ).getTime();
        </script>
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        
        <meta name="renderer" content="webkit">
        <meta name="google-site-verification" content="oQXrGa_mTgxg7joO0himE0QuFeqOVmm-SDC1H2dzT4c">
        <meta name="baidu-site-verification" content="wibJopuIuI" />
        <meta name="viewport" content="width=device-width, initial-scale=1">
        
        <meta name="keywords" content="WE|人人贷|WE公司|WE平台|WE网站|小白|基金|指数型基金|股票型基金|混合型基金|债券型基金|货币基金|余额宝|理财通|开放式基金净值|每日基金净值|基金净值查询|最新基金净值|全部基金净值">
        <meta name="description" content="人人贷WE理财(Wealth Evolution)甄选基金，去粗取精，满足用户多元投资需求，提供一站式理财平台。WE官网每日提供最新的开放式基金（股票型/指数型/债券型/混合型/货币型基金）净值，为用户提供更加高效、便捷、专业、专属的全新理财服务。">
        <title>基金列表-人人贷WE理财(Wealth Evolution)官网</title>
    
        <link rel="shortcut icon" type="image/x-icon" href="/static/common/img/favicon.ico" />
        <link rel="stylesheet" type="text/css" href="/TP/one/Public/css/base_ac0da34.css">
        <!--header中的css-->
        
        
        <script>
            !function(){var c=Object.prototype.toString;var a=/complete|loaded|interactive/;var m="m.we.com/s1/w.gif";var d="";var p=location.protocol;var k="";var l=p+"//"+m;function b(q){return c.call(q)==="[object String]"}function j(s,r){for(var q in r){if(r.hasOwnProperty(q)){s[q]=r[q]}}}function o(s){var q="";for(var r in s){q+=r+"="+encodeURIComponent(s[r])+"&"}return q}function g(t){var s=(new Date()).getTime();var r="___log_"+s;var q=new Image();window[r]=q;q.onload=q.onerror=function(){q.onload=q.onerror=null;window[r]=null;q=null};q.src=t+"&_r="+s}function f(t){var q={pl:d,pid:k};for(var s in t){if(t.hasOwnProperty(s)){if(!b(t[s])){q[s]=JSON.stringify(t[s])}else{q[s]=t[s]}}}var r=l+"?"+o(q);g(r)}var n=false;var i={};var e={init:function(q){d=q.platform;k=q.pageID;if(!k){k=location.pathname}},perf:{headStart:function(q){i.jhead_start=q;return e},bodyStart:function(q){q=q||(new Date()).getTime();i.jbody_start=q;return e},bodyEnd:function(q){q=q||(new Date()).getTime();i.jbody_end=q;return e},domReady:function(q){if(i.jdom_ready){return}q=q||(new Date()).getTime();i.jdom_ready=q;return e},fullLoad:function(q){q=q||(new Date()).getTime();i.jfull_load=q;return e},send:function(){if(!n){if(window.performance&&window.performance.timing&&typeof window.performance.timing.toJSON==="function"){j(i,window.performance.timing.toJSON())}f({perf:i})}n=true}}};if(a.test(document.readyState)&&document.body){e.perf.domReady()}else{if(typeof document.addEventListener==="function"){document.addEventListener("DOMContentLoaded",function(){e.perf.domReady()})}}window.weLogger=e;function h(){e.perf.fullLoad();e.perf.send()}if(document.readyState!=="complete"){if(typeof window.addEventListener==="function"){window.addEventListener("load",h)}else{if(window.attachEvent){window.attachEvent("onload",h)}}}}();
        </script>
        <script src="/TP/one/Public/js/vendors_8a6040f.js"></script>
        <!--header中的js-->
        
        <script>
            weLogger.init({ platform : 'pc' });
            weLogger.perf.headStart( _jHeadStart );
        </script>
    <link rel="stylesheet" type="text/css" href="/TP/one/Public/css/common_widget_a1e249e.css" /><link rel="stylesheet" type="text/css" href="/TP/one/Public/css/fund_widget_823e06a.css" /></head>
    <body class="body">
        <script>
            weLogger.perf.bodyStart( );
        </script>
        
        
        
            
<div class="wdg-top-header">
    <div class="main-section">
        <span class="tel-phone"><i class="icon-phone"></i>客服电话: 400-090-6600</span>
        <ul class="site-nav">
            
                <li class="nav-item"><a target="_self" href="/loginPage.action">立即登录</a></li>
                <li class="nav-item-split"></li>
                <li class="nav-item"><a target="_self" href="/regPage.action?registerSource=web_top">快速注册</a></li>
                <li class="nav-item-split"></li>
            
            
            <li class="nav-item"><a target="_blank" href="/about/about.action?flag=intro">关于我们</a></li>
            <li class="nav-item-split"></li>
            <li class="nav-item"><a target="_blank" href="/help/index.action">帮助中心</a></li>
            <li class="nav-item-split"></li>
            <li class="nav-item"><a target="_blank" href="http://bbs.we.com/">理财论坛</a></li>
            <li class="nav-item-split"></li>
            <li class="nav-item"><i class="icon-mobile3"></i><a target="_blank" href="/event/app.action">移动客户端</a></li>
        </ul>
    </div>
</div>
            
<div class="wdg-second-header">
    <div class="main-section">
        <a href="/" class="brand-logo mt15">
            <img src="/TP/one/Public/images/we-logo_79fb26f.png" alt="logo" />
        </a>
        <ul class="site-nav">
            <li class="user-item fn-clear">
                
                    <div class="user-avatar-container fn-left">
                        <img src="/TP/one/Public/images/.com/fund/info/tag000001/data:image/png;base64,"  />
                        <div class="avatar-masking"><a href="/account/index.action"></a></div>
                    </div>
                    <div class="user-name fn-clear">
                        <a href="/loginPage.action">我的账户</a>
                    </div>
                
                
            </li>
            <li class="channel-item"><a href="/TP/one/index.php/home/index">首页</a></li>
            <li class="channel-item"><a href="/TP/one/index.php/home/index/jijin">基金</a></li>
            <li class="channel-item"><a href="/TP/one/index.php/home/index/ujihua">U计划</a></li>
            <li class="channel-item"><a href="/TP/one/index.php/home/index/xinjihua">薪计划</a></li>


            <li class="channel-item"><a href="/TP/one/index.php/home/index/zq">债权</a></li>
            <li class="channel-item"><a href="/TP/one/index.php/home/index/xinshou">新手专区</a></li>
             
               
                
                    
                        
                    
                    
                        
                    
                    
                        
                    
                
             
        </ul>
    </div>
</div>
<div id="header-helper" style="display: none;">
    <span id="header-helper-authenticated">
      
      false
    </span>
</div>
        
        
            
        
        
            
        
        <div id="pg-server-message" data-status="" data-message="" data-ispop="" data-fundTips="" style="display: none;"></div>
        <div class="maincontent">
            
         <!-- @require "common:widget/oui/oui.css"-->
        <!--轮播图-->
        
	 <div class="color-white-bg" id="fund-list-header">
    	<div class="slides" id="slidebox">

  <ul id="slides" class="ui-slides">

    	<li class="ui-slide01 fn-hide" data-background="url(/upload/cms/page/fundBanner/218452b5-46eb-4c43-8b67-2bf373016297.jpg?20140401a) no-repeat 50% 50%" style="background-image: url(/upload/cms/page/fundBanner/218452b5-46eb-4c43-8b67-2bf373016297.jpg?20140401a); display: none; background-position: 50% 50%; background-repeat: no-repeat no-repeat;">

          <a  href="http://www.we.com/event/eventPage.action?type=assetsAllocation2"  target="_blank"></a>

        </li>

	<li class="ui-slide01 fn-hide" data-background="url(/upload/cms/page/fundBanner/28ba9e8b-d315-41fe-86e4-dc2c6e56abee.jpg?20140401a) no-repeat 50% 50%" style="background-image: url(/upload/cms/page/fundBanner/28ba9e8b-d315-41fe-86e4-dc2c6e56abee.jpg?20140401a); display: none; background-position: 50% 50%; background-repeat: no-repeat no-repeat;">

          <a  href="http://www.we.com/fund/fundToPageAction!toFundIntro.action?fundType=hybrid"  target="_blank"></a>

        </li>

  </ul>

</div>
	    <div class="main-section">
	        <div id="slider_nav">
	        </div>
	    </div>
 </div>
        <!--基金分类-->
         
 
 <div class="container_12_1080 mt14 color-white-bg" id="fund-list-filter">
 	 <ul class="ui-fund-filter fn-clear">
 	 	<li class="fn-left w268 border-left-gray border-right-gray bg-1"></li>
 	 	<li class="fn-left w269 border-right-gray bg-2"></li>
 	 	<li class="fn-left w269 border-right-gray bg-3"></li>
 	 	<li class="fn-left w269 border-right-gray bg-4"></li>
 	 </ul>
 </div>
        <!--基金tab-->
        
<div class="container_12_1080 mt14 color-white-bg" id="fund-list-main">
    <div class="ui-tab ui-tab-transparent" id="fundlist-tab">
      <ul class="ui-tab-items fn-clear">
		<li class="ui-tab-item ui-tab-item-5  ui-tab-item-current " data-name="FUND_MIX">
			<a class="ui-tab-item-link">混合型</a>
		</li>
		<li class="ui-tab-item ui-tab-item-5 " data-name="FUND_BOND">
			<a class="ui-tab-item-link">债券型</a>
		</li>
		<li class="ui-tab-item ui-tab-item-5 " data-name="FUND_MONEY">
			<a class="ui-tab-item-link">货币型</a>
		</li>
        <li class="ui-tab-item ui-tab-item-5 " data-name="FUND_INDEX">
          <a class="ui-tab-item-link">指数型</a>
        </li>
        <li class="ui-tab-item ui-tab-item-5 " data-name="FUND_STOCK">
          <a class="ui-tab-item-link">股票型</a>
        </li>
      
      </ul>
    </div>  
    <div id="fundlist-tab-content" class="ui-tab ui-tab-transparent">
     	<div class="ui-tab-content " data-name="FUND_INDEX">
     		<div class="fund-tab-info">跟踪大盘走势，懒人投资首选</div>
             
	         <div class="ui-fund-list" id="fundBreakEven-index"></div>
     		<!--列表数据展示一条 begin-->
     		<ul class="ui-fund-list" id="fundlist-index"></ul>
   		    <!--列表数据展示一条 end-->     		
   		   <div class="pagination-box fn-clear" id="fundlist-index-pagination" ></div> 
     	</div>
     	<div class="ui-tab-content " data-name="FUND_STOCK">
     		<div class="fund-tab-info">高风险、高收益，赚钱利器</div>
             
	         <div class="ui-fund-list" id="fundBreakEven-stock"></div>
     		<!--列表数据展示一条 begin-->
     		<ul class="ui-fund-list" id="fundlist-stock"></ul>
   		    <!--列表数据展示一条 end-->     		
   		   <div class="pagination-box fn-clear" id="fundlist-stock-pagination" ></div>
     	</div>
     	<div class="ui-tab-content  ui-tab-content-current " data-name="FUND_MIX">
     		<div class="fund-tab-info">仓位灵活，进可获取收益，退可控制风险</div>
	         <div class="ui-fund-list" id="fundBreakEven-mix"></div>
     		<!--列表数据展示一条 begin-->
     		<ul class="ui-fund-list" id="fundlist-mix"></ul>
   		    <!--列表数据展示一条 end-->     		
   		   <div class="pagination-box fn-clear" id="fundlist-mix-pagination" ></div>
     	</div>
     	<div class="ui-tab-content " data-name="FUND_BOND">
     		<div class="fund-tab-info">纯债稳扎稳打，混债攻守兼备</div>
             
	         <div class="ui-fund-list" id="fundBreakEven-bond"></div>
     		<!--列表数据展示一条 begin-->
     		<ul class="ui-fund-list" id="fundlist-bond"></ul>
   		    <!--列表数据展示一条 end-->     		
   		   <div class="pagination-box fn-clear" id="fundlist-bond-pagination" ></div>
     	</div>
     	<div class="ui-tab-content " data-name="FUND_MONEY">
     		<div class="fund-tab-info">收益不错的活期理财产品</div>
             
	         <div class="ui-fund-list" id="fundBreakEven-money"></div>
     		<!--列表数据展示一条 begin-->
     		<ul class="ui-fund-list" id="fundlist-money"></ul>
   		    <!--列表数据展示一条 end-->     		
   		   <div class="pagination-box fn-clear" id="fundlist-money-pagination" ></div>
     	</div>
      <div class="fundtip"><p>注：基金销售服务由珠海盈米财富管理有限公司提供，该公司基金销售资格证书编号为000000378。</p></div>     	
     </div>
 </div>
        <!--模板-->
        
<script id="fundlist-index-template" type="text/x-handlebars-template">
{{# each resultList }}
 <li class="ui-fund-list-li fn-clear padding-left-25">
			     	<div  class="fn-left fund-info-left width175">
			     			<ul class="fund-info-left-main">
			     				<li class="fund-value-name">最新净值</li>
			     				<li class="fund-value {{navColor}}"><span class="num-family">{{ nav }}</span><i>元</i></li>
			     				<li class="fund-rate">净值增长率:{{rateGrowth}}</li>
			     			</ul>
			     	</div>
			     	<div class="fn-right fund-info-right" >
			     			<ul class="fund-info-right-top  fn-clear">
			     				<li class="fund-info-right-top-1 fn-left">
			     					<a  target="_blank" href="/fund/info/{{fundCode}}-tag010002">{{abbrev}}<i>{{fundCode}}</i></a>
			     					{{# if remark }}
			     					<span class="risk-level ml10">{{remark}}</span>
			     					{{/ if }}
			     				</li>
			     				<li class="fund-info-right-top-2 fn-right fn-clear">
			     					<span class="ml15 fn-right">申购费率<i>{{fee}}</i>%</span>
			     					<span class="fn-right">{{minSubscript}}元起</span>
			     				</li>
			     			</ul>
			     			<ul class="fund-info-right-bottom">
			     				<li class="fn-clear">
			     					<ul class="fund-time-all fn-left w140">
			     						<li class="fund-time">近一个月</li>
			     						<li class="fund-time-value">{{rateMonth}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w154">
			     						<li class="fund-time">近三个月</li>
			     						<li class="fund-time-value">{{rateSeason}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w148">
			     						<li class="fund-time">近半年</li>
			     						<li class="fund-time-value">{{rateHalfYear}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w153">
			     						<li class="fund-time">近一年</li>
			     						<li class="fund-time-value">{{rateYear}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w127">
			     						<li class="fund-time">今年以来</li>
			     						<li class="fund-time-value">{{thisYear}}</li>
			     					</ul>
 										{{# if sbPeriod458 }}
											<a class="fn-right fund-btn-gray fund-btn-position" href="javascript:void(0)"> 停止申购 </a>
										{{/if }}
										{{# if sbPeriod9 }}
											<a class="fn-right fund-btn-gray fund-btn-position" href="javascript:void(0)">封闭期</a>
										{{/if }}
										{{# if sbPeriod06 }}
											<a class="fn-right fund-btn fund-btn-position"
											target="_blank" href="/fund/info/{{fundCode}}-tag010001">申购</a>
										{{/ if }}
  										{{# if sbPeriod1 }}
											<a class="fn-right fund-btn fund-btn-position"
											target="_blank" href="/fund/info/{{fundCode}}-tag010001">认购</a>
										{{/ if }}
			     				</li>
			     			</ul>
			     	</div>
     			</li>
{{ else }}
<li class="color-gray-text text-center" style="font-size:18px; padding-top:20px;padding-bottom:20px">
     	{{# if _message }}{{ _message }}{{ else }}指数型基金没有数据  {{/ if }}
</li>
{{/ each }}
</script>
<script id="fundlist-stock-template" type="text/x-handlebars-template">
{{# each resultList }}
 <li class="ui-fund-list-li fn-clear padding-left-25">
			     	<div  class="fn-left fund-info-left width149">
			     			<ul class="fund-info-left-main">
			     				<li class="fund-value-name">最新净值</li>
			     				<li class="fund-value {{navColor}}"><span class="num-family">{{ nav }}</span><i>元</i></li>
			     				<li class="fund-rate">净值增长率:{{rateGrowth}}</li>
			     			</ul>
			     	</div>
			     	<div class="fn-right fund-info-right" >
			     			<ul class="fund-info-right-top  fn-clear">
			     				<li class="fund-info-right-top-1 fn-left">
			     					<a  target="_blank" href="/fund/info/{{fundCode}}-tag010002">{{abbrev}}<i>{{fundCode}}</i></a>
			     					{{# if remark }}
			     					<span class="risk-level ml10">{{remark}}</span>
			     					{{/ if }}
			     				</li>
			     				<li class="fund-info-right-top-2 fn-right fn-clear">
			     					<span class="ml15 fn-right">申购费率<i>{{fee}}</i>%</span>
			     					<span class="fn-right">{{minSubscript}}元起</span>
			     				</li>
			     			</ul>
			     			<ul class="fund-info-right-bottom">
			     				<li class="fn-clear">
			     					<ul class="fund-time-all fn-left w140">
			     						<li class="fund-time">近一个月</li>
			     						<li class="fund-time-value">{{rateMonth}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w154">
			     						<li class="fund-time">近三个月</li>
			     						<li class="fund-time-value">{{rateSeason}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w148">
			     						<li class="fund-time">近半年</li>
			     						<li class="fund-time-value">{{rateHalfYear}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w153">
			     						<li class="fund-time">近一年</li>
			     						<li class="fund-time-value">{{rateYear}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w127">
			     						<li class="fund-time">今年以来</li>
			     						<li class="fund-time-value">{{thisYear}}</li>
			     					</ul>
 										{{# if sbPeriod458 }}
											<a class="fn-right fund-btn-gray fund-btn-position" href="javascript:void(0)"> 停止申购 </a>
										{{/if }}
										{{# if sbPeriod9 }}
											<a class="fn-right fund-btn-gray fund-btn-position" href="javascript:void(0)">封闭期</a>
										{{/if }}
										{{# if sbPeriod06 }}
											<a class="fn-right fund-btn fund-btn-position"
											target="_blank" href="/fund/info/{{fundCode}}-tag010001">申购</a>
										{{/ if }}
  										{{# if sbPeriod1 }}
											<a class="fn-right fund-btn fund-btn-position"
											target="_blank" href="/fund/info/{{fundCode}}-tag010001">认购</a>
										{{/ if }}
			     				</li>
			     			</ul>
			     	</div>
     			</li>
{{ else }}
<li class="color-gray-text text-center" style="font-size:18px; padding-top:20px;padding-bottom:20px">
     	{{# if _message }}{{ _message }}{{ else }}股票型基金没有数据  {{/ if }}
</li>
{{/ each }}
</script>
<script id="fundlist-mix-template" type="text/x-handlebars-template">
{{# each resultList }}
 <li class="ui-fund-list-li fn-clear padding-left-56">
	<div  class="fn-left fund-info-left width149">
			     			<ul class="fund-info-left-main">
			     				<li class="fund-value-name">最新净值</li>
			     				<li class="fund-value {{navColor}}"><span class="num-family">{{ nav }}</span><i>元</i></li>
			     				<li class="fund-rate">净值增长率:{{rateGrowth}}</li>
			     			</ul>
			     	</div>
			     	<div class="fn-right fund-info-right" >
			     			<ul class="fund-info-right-top  fn-clear">
			     				<li class="fund-info-right-top-1 fn-left">
			     					<a  target="_blank" href="/fund/info/{{fundCode}}-tag010002">{{abbrev}}<i>{{fundCode}}</i></a>
			     					{{# if remark }}
			     					<span class="risk-level ml10">{{remark}}</span>
			     					{{/ if }}
			     				</li>
			     				<li class="fund-info-right-top-2 fn-right fn-clear">
			     					<span class="ml15 fn-right">申购费率<i>{{fee}}</i>%</span>
			     					<span class="fn-right">{{minSubscript}}元起</span>
			     				</li>
			     			</ul>
			     			<ul class="fund-info-right-bottom">
			     				<li class="fn-clear">
			     					<ul class="fund-time-all fn-left w140">
			     						<li class="fund-time">近一个月</li>
			     						<li class="fund-time-value">{{rateMonth}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w154">
			     						<li class="fund-time">近三个月</li>
			     						<li class="fund-time-value">{{rateSeason}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w148">
			     						<li class="fund-time">近半年</li>
			     						<li class="fund-time-value">{{rateHalfYear}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w153">
			     						<li class="fund-time">近一年</li>
			     						<li class="fund-time-value">{{rateYear}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w127">
			     						<li class="fund-time">今年以来</li>
			     						<li class="fund-time-value">{{thisYear}}</li>
			     					</ul>
 										{{# if sbPeriod458 }}
											<a class="fn-right fund-btn-gray fund-btn-position" href="javascript:void(0)"> 停止申购 </a>
										{{/if }}
										{{# if sbPeriod9 }}
											<a class="fn-right fund-btn-gray fund-btn-position" href="javascript:void(0)">封闭期</a>
										{{/if }}
										{{# if sbPeriod06 }}
											<a class="fn-right fund-btn fund-btn-position"
											target="_blank" href="/fund/info/{{fundCode}}-tag010001">申购</a>
										{{/ if }}
  										{{# if sbPeriod1 }}
											<a class="fn-right fund-btn fund-btn-position"
											target="_blank" href="/fund/info/{{fundCode}}-tag010001">认购</a>
										{{/ if }}
			     				</li>
			     			</ul>
			     	</div>
     			</li>
{{ else }}
<li class="color-gray-text text-center" style="font-size:18px; padding-top:20px;padding-bottom:20px">
     	{{# if _message }}{{ _message }}{{ else }}混合型基金没有数据  {{/ if }}
</li>
{{/ each }}
</script>
<script id="fundlist-bond-template" type="text/x-handlebars-template">
{{# each resultList }}
 <li class="ui-fund-list-li fn-clear padding-left-25">
			     	<div  class="fn-left fund-info-left width175">
			     			<ul class="fund-info-left-main">
			     				<li class="fund-value-name">最新净值</li>
			     				<li class="fund-value {{navColor}}"><span class="num-family">{{ nav }}</span><i>元</i></li>
			     				<li class="fund-rate">净值增长率:{{rateGrowth}}</li>
			     			</ul>
			     	</div>
			     	<div class="fn-right fund-info-right" >
			     			<ul class="fund-info-right-top  fn-clear">
			     				<li class="fund-info-right-top-1 fn-left">
			     					<a  target="_blank" href="/fund/info/{{fundCode}}-tag010002">{{abbrev}}<i>{{fundCode}}</i></a>
			     					{{# if remark }}
			     					<span class="risk-level ml10">{{remark}}</span>
			     					{{/ if }}
			     				</li>
			     				<li class="fund-info-right-top-2 fn-right fn-clear">
			     					<span class="ml15 fn-right">申购费率<i>{{fee}}</i>%</span>
			     					<span class="fn-right">{{minSubscript}}元起</span>
			     				</li>
			     			</ul>
			     			<ul class="fund-info-right-bottom">
			     				<li class="fn-clear">
			     					<ul class="fund-time-all fn-left w140">
			     						<li class="fund-time">近一个月</li>
			     						<li class="fund-time-value">{{rateMonth}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w154">
			     						<li class="fund-time">近三个月</li>
			     						<li class="fund-time-value">{{rateSeason}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w148">
			     						<li class="fund-time">近半年</li>
			     						<li class="fund-time-value">{{rateHalfYear}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w153">
			     						<li class="fund-time">近一年</li>
			     						<li class="fund-time-value">{{rateYear}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w127">
			     						<li class="fund-time">今年以来</li>
			     						<li class="fund-time-value">{{thisYear}}</li>
			     					</ul>
	 								    {{# if sbPeriod458 }}
											<a class="fn-right fund-btn-gray fund-btn-position" href="javascript:void(0)"> 停止申购 </a>
										{{/if }}
										{{# if sbPeriod9 }}
											<a class="fn-right fund-btn-gray fund-btn-position" href="javascript:void(0)">封闭期</a>
										{{/if }}
										{{# if sbPeriod06 }}
											<a class="fn-right fund-btn fund-btn-position"
											target="_blank" href="/fund/info/{{fundCode}}-tag010001">申购</a>
										{{/ if }}
  										{{# if sbPeriod1 }}
											<a class="fn-right fund-btn fund-btn-position"
											target="_blank" href="/fund/info/{{fundCode}}-tag010001">认购</a>
										{{/ if }}
			     				</li>
			     			</ul>
			     	</div>
     			</li>
{{ else }}
<li class="color-gray-text text-center" style="font-size:18px; padding-top:20px;padding-bottom:20px">
     	{{# if _message }}{{ _message }}{{ else }}债券型基金没有数据  {{/ if }}
</li>
{{/ each }}
</script>
<script id="fundlist-money-template" type="text/x-handlebars-template">
{{# each resultList }}
 <li class="ui-fund-list-li fn-clear padding-left-25">
			     	<div  class="fn-left fund-info-left width175">
			     			<ul class="fund-info-left-main">
			     				<li class="fund-value-name">万份收益</li>
			     				<li class="fund-value {{profit10kColor}}"><span class="num-family ">{{ profit10k }}</span><i>元</i></li>
			     				<li class="fund-rate">七日年化收益率:{{rate7day}}%</li>
			     			</ul>
			     	</div>
			     	<div class="fn-right fund-info-right" >
			     			<ul class="fund-info-right-top  fn-clear">
			     				<li class="fund-info-right-top-1 fn-left">
			     					<a  target="_blank" href="/fund/info/{{fundCode}}-tag010002">{{abbrev}}<i>{{fundCode}}</i></a>
			     					{{# if remark }}
			     					<span class="risk-level ml10">{{remark}}</span>
			     					{{/ if }}
			     				</li>
			     				<li class="fund-info-right-top-2 fn-right fn-clear">
			     					<span class="ml15 fn-right">申购费率<i>{{fee}}</i>%</span>
			     					<span class="fn-right">{{minSubscript}}元起</span>
			     				</li>
			     			</ul>
			     			<ul class="fund-info-right-bottom">
			     				<li class="fn-clear">
			     					<ul class="fund-time-all fn-left w140">
			     						<li class="fund-time">近一个月</li>
			     						<li class="fund-time-value">{{rateMonth}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w154">
			     						<li class="fund-time">近三个月</li>
			     						<li class="fund-time-value">{{rateSeason}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w148">
			     						<li class="fund-time">近半年</li>
			     						<li class="fund-time-value">{{rateHalfYear}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w153">
			     						<li class="fund-time">近一年</li>
			     						<li class="fund-time-value">{{rateYear}}</li>
			     					</ul>
			     					<ul class="fund-time-all fn-left w127">
			     						<li class="fund-time">今年以来</li>
			     						<li class="fund-time-value">{{thisYear}}</li>
			     					</ul>
 										{{# if sbPeriod458 }}
											<a class="fn-right fund-btn-gray fund-btn-position" href="javascript:void(0)"> 停止申购 </a>
										{{/if }}
										{{# if sbPeriod9 }}
											<a class="fn-right fund-btn-gray fund-btn-position" href="javascript:void(0)">封闭期</a>
										{{/if }}
										{{# if sbPeriod06 }}
											<a class="fn-right fund-btn fund-btn-position"
											target="_blank" href="/fund/info/{{fundCode}}-tag010001">申购</a>
										{{/ if }}
  										{{# if sbPeriod1 }}
											<a class="fn-right fund-btn fund-btn-position"
											target="_blank" href="/fund/info/{{fundCode}}-tag010001">认购</a>
										{{/ if }}
			     				</li>
			     			</ul>
			     	</div>
     			</li>
{{ else }}
<li class="color-gray-text text-center" style="font-size:18px; padding-top:20px;padding-bottom:20px">
     	{{# if _message }}{{ _message }}{{ else }}货币型基金没有数据 {{/ if }}
</li>
{{/ each }}
</script>
        
<script id="fundBreakEven-template" type="text/x-handlebars-template">
	{{# each resultList }}
	{{# if isTop }}
	{{# if isSafe }}
	<!--保本基金 begin-->
	<li class="fund-break-event-items">
		<div class="break-even-logo"></div>
		<div class="fund-break-event-item fn-clear">
		<div  class="fn-left fund-info-left">
			<div class="fund-info-left-main">
				<div class="fund-value">
						<div class="fund-big-title">保本基金</div>
						<div class="fund-hr">
							<i></i><span class="icon-diamonds"></span><i></i>
						</div>
					<span>本金有保&nbsp;安而有盈</span>
				</div>
			</div>
		</div>
		<div class="fn-right fund-info-right" >
			<ul class="fund-info-right-top fund-time-all-padding">
				<li class="fund-brak-even-title">
                    <a  target="_blank" href="/fund/info/{{fundCode}}-tag010002">{{abbrev}}<i>{{fundCode}}</i></a>
                    		{{# if remark }}
			     					<span class="risk-level ml10">{{remark}}</span>
			     			{{/ if }}
                     </li>
			</ul>
			<ul class="fund-info-right-bottom  ">
					<li class="fn-clear">
						<ul class="fund-time-all fund-time-all-padding fn-left w185">
							<li class="fund-time-value fund-time-value-orange">
							{{# if isExpected }}
								<span class="orange-font num-family">{{expected}}</span><span>%以上</span>
							{{/ if }}
							{{# if isNoteExpected }}
							<span class="orange-font">--</span>
							{{/ if }}
							</li>
							<li class="fund-time">保本期收益</li>
						</ul>
						<ul class="fund-time-all fund-time-all-padding fn-left w185">
							<li class="fund-time-value"><span  class="black-font num-family">{{safeMoneyMonth}}</span><span>个月</span></li>
							<li class="fund-time">最长保本周期</li>
						</ul>
						<ul class="fund-time-all fund-time-all-padding fn-left w252">
							<li class="fund-time-value"><span  class="black-font  num-family">{{endBuyDate}}</span></li>
							<li class="fund-time">认购截止日期</li>
						</ul>
						<ul class="fund-time-all   fn-left w80">
							<li class="fund-brak-btn-wrap">
 										{{# if sbPeriod458 }}
											<a class="fn-right fund-btn-gray fund-btn-position" href="javascript:void(0)"> 停止申购 </a>
										{{/if }}
										{{# if sbPeriod9 }}
											<a class="fn-right fund-btn-gray fund-btn-position" href="javascript:void(0)">封闭期</a>
										{{/if }}
										{{# if sbPeriod06 }}
											<a class="fn-right fund-btn fund-btn-position"
											target="_blank" href="/fund/info/{{fundCode}}-tag010001">申购</a>
										{{/ if }}
  										{{# if sbPeriod1 }}
											<a class="fn-right fund-btn fund-btn-position"
											target="_blank" href="/fund/info/{{fundCode}}-tag010001">认购</a>
										{{/ if }}
								   <span class="fund-brak-btn-text-position">{{last}}</span>
							</li>
						</ul>
			</ul>
		</div>
		</div>
	</li><!--保本基金 end-->
    {{else}}
		<li class="ui-fund-list-li fn-clear padding-left-56">
				<div class="break-even-logo"></div>
				<div  class="fn-left fund-info-left width149">
					<ul class="fund-info-left-main">
						<li class="fund-value-name">最新净值</li>
						<li class="fund-value {{navColor}}"><span class="num-family">{{ nav }}</span><i>元</i></li>
						<li class="fund-rate">净值增长率:{{rateGrowth}}</li>
						</ul>
					</div>
					<div class="fn-right fund-info-right" >
					<ul class="fund-info-right-top  fn-clear">
						<li class="fund-info-right-top-1 fn-left">
						<a  target="_blank" href="/fund/info/{{fundCode}}-tag010002">{{abbrev}}<i>{{fundCode}}</i></a>
						{{# if remark }}
						<span class="risk-level ml10">{{remark}}</span>
						{{/ if }}
						</li>
						<li class="fund-info-right-top-2 fn-right fn-clear">
						<span class="ml15 fn-right">申购费率<i>{{fee}}</i>%</span>
						<span class="fn-right">{{minSubscript}}元起</span>
					</li>
					</ul>
					<ul class="fund-info-right-bottom">
					<li class="fn-clear">
					<ul class="fund-time-all fn-left w140">
						<li class="fund-time">近一个月</li>
						<li class="fund-time-value">{{rateMonth}}</li>
					</ul>
					<ul class="fund-time-all fn-left w154">
						<li class="fund-time">近三个月</li>
						<li class="fund-time-value">{{rateSeason}}</li>
					</ul>
					<ul class="fund-time-all fn-left w148">
						<li class="fund-time">近半年</li>
						<li class="fund-time-value">{{rateHalfYear}}</li>
					</ul>
					<ul class="fund-time-all fn-left w153">
						<li class="fund-time">近一年</li>
						<li class="fund-time-value">{{rateYear}}</li>
					</ul>
					<ul class="fund-time-all fn-left w127">
						<li class="fund-time">今年以来</li>
						<li class="fund-time-value">{{thisYear}}</li>
					</ul>
										{{# if sbPeriod458 }}
											<a class="fn-right fund-btn-gray fund-btn-position" href="javascript:void(0)"> 停止申购 </a>
										{{/if }}
										{{# if sbPeriod9 }}
											<a class="fn-right fund-btn-gray fund-btn-position" href="javascript:void(0)">封闭期</a>
										{{/if }}
										{{# if sbPeriod06 }}
											<a class="fn-right fund-btn fund-btn-position"
											target="_blank" href="/fund/info/{{fundCode}}-tag010001">申购</a>
										{{/ if }}
  										{{# if sbPeriod1 }}
											<a class="fn-right fund-btn fund-btn-position"
											target="_blank" href="/fund/fundInfoAction!fundinfo.action?source=tag010001&fundCode={{fundCode}}">认购</a>
										{{/ if }}
				</li>
				</ul>
			</div>
	</li>
	   {{/ if }}
	{{/ if }}
 {{/ each }}
</script>
    
        </div>
        <div class="pg-container-content"></div>
        
            
<div class="ui-footer" id="footer">
  <div class="main-section">
    <div class="ui-footer-section ui-footer-narrow-hide fn-clear" style="width: 1150px;">

  <h4 class="color-gray-text text-big fn-left w100">友情链接</h4>

  <ul class="ui-footer-links fn-left friendlink">

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.we.com">人人贷</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://bbs.we.com">理财论坛</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.rong360.com/licai/">融360理财</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://bbs.kjj.com">酷基金论坛</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://fenlei.baike.com">微百科</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.asiafinance.cn">亚洲财经</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.wangdaiabc.cn">网贷ABC</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.dimeng.net">P2P系统</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.chuangtianxia.cn ">创业者</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.angelclub.com">天使客</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.tianshijie.com.cn/">天使街</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://news.smm.cn">贵金属资讯</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.wangdaigw.com   ">网贷港湾</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.wquant.com ">微量网</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.chemayi.com">互联网车险</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.trust-one.com">信托投资</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.jiedai.cn"> 借贷网  </a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.dib66.com">地标金融 </a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.cpiaoju.com">云票据</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://99dai.cn">99贷</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.lvhuadai.com">绿化贷</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.qianxiangbank.com">钱香金融</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.cnaidai.com">爱贷网</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.tuandai.com">团贷网</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.auagm.com">贵金属公司</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.hhrfs.com ">合伙人</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.yunxindai.com">云信贷理财</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.btctrade.com">比特币</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://group.ppdai.com">拍拍贷</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.ycb.com">银承宝</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="https://www.tzg.cn/">铜掌柜</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.renrencaopan.com">人人操盘</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.wangdaishequ.cn/">网贷社区</a>

    </li>

    <li class="fn-left">

      <a class="gray" target="_blank" href="http://www.daicash.com">贷金所</a>

    </li>

  </ul>

</div>

    <i class="icon icon-footer-snow-down"></i>
    <div class="ui-footer-section ui-footer-links second">
      <ul class="fn-clear">
        <li class="fn-left">
          <h4 class="color-gray-text text-big w100">
            <a class="gray" target="_blank" href="/about/about.action?flag=intro">公司介绍</a></h4>
        </li>
        <li class="fn-left">
          <a class="gray" target="_blank" href="/guide/investSecurity.action">安全保障</a>
        </li>
        <li class="fn-left">
          <a class="gray" target="_blank" href="/about/about.action?flag=report">媒体报道</a>
        </li>
        <li class="fn-left">
          <a class="gray" target="_blank" href="/about/about.action?flag=responsibility">社会责任</a>
        </li>
        <li class="fn-left">
          <a class="gray" target="_blank" href="/about/about.action?flag=invite">招贤纳士</a>
        </li>
        
        <li class="fn-left">
          <a class="gray" target="_blank" href="/help/index.action">帮助中心</a>
        </li>
        <li class="fn-left">
          <a class="gray" target="_blank" href="/about/about.action?flag=contact">联系我们</a>
        </li>
      </ul>
      <ul class="fn-clear icons">
        <li class="fn-left">
          <h4 class="color-gray-text text-big w100">客户服务</h4>
        </li>
        <li class="fn-left"><a id="wexinBtn" class="gray" href="javascript:;" tabindex="-1">
          <i class="icon we-chat"></i>微信</a></li>
        <li class="fn-left"><a class="gray" target="_blank" href="http://e.weibo.com/renrendai?ref=http%3A%2F%2Fwww.renrendai.com%2F"><i class="icon weibo"></i>新浪微博</a></li>
        <li class="fn-left"><a class="gray cursor-pointer" target="_blank" onclick="online_service();"><i class="icon online-customer-service"></i>在线客服</a></li>
      </ul>
      <ul class="fn-clear service">
        <li class="fn-left">
          <h4 class="color-gray-text text-big w100">客服电话</h4>
        </li>
        <li class="fn-left ui-footer-phone-number">400-090-6600</li>
        <li class="fn-left">9:00 - 21:00</li>
      </ul>
      <div class="weixin fn-clear">
        <dl class="fn-left">
          <dd><img src="/TP/one/Public/images/we-weixing_334069c.png" alt=""></dd>
          <dt>关注微信账号</dt>
        </dl>
        <dl class="fn-left app-down">
          <dd><img src="/TP/one/Public/images/app-weixin_32107fa.png" alt=""></dd>
          <dt>下载手机app</dt>
        </dl>
      </div>
    </div>
    <div class="ui-footer-section">
      <div class="ui-footer-copyright">
        <span class="ui-footer-contact-link color-gray-text">© 2016 人人贷 All rights reserved</span>
        <span class="ui-footer-contact-link color-gray-text has-separator">人人贷商务顾问(北京)有限公司</span>
        <span class="ui-footer-contact-link color-gray-text has-separator"><a class="gray" target="_blank" href="/icp/icp.html">京ICP证 100953号</a></span>
        <span class="ui-footer-contact-link color-gray-text has-separator">京公网安备11010502020657</span>
        <span class="ui-footer-contact-link color-gray-text has-separator last">京ICP备12025643号-1</span>
      </div>
      <div class="ui-footer-verification fn-clear">
        <a class="ui-footer-verification-item fn-left credibility" title="人人贷WE理财荣获中国电子商务协会“诚信网站”认证殊荣" id='___szfw_logo___' href='https://credit.szfw.org/CX20151016011685230181.html' target='_blank'></a>
        <a class="ui-footer-verification-item fn-left trust" id="kx_verify" href="https://ss.knet.cn/verifyseal.dll?sn=e15112011010561538m3ux000000&ct=df&a=1&pa=0.6757671611849219"title="人人贷WE理财已通过中网权威数据库对比，获得“可信网站”身份验证，您可放心使用。"  target="_blank">
        </a>
        <a class="ui-footer-verification-item fn-left internet" href="http://si.trustutn.org/info?sn=339151201000317676777" title="人人贷WE理财已通过认证联盟权威数据库对比，获得“互联网金融行业认证”，您可放心使用。" target="_blank"></a>
        <a class="ui-footer-verification-item fn-left itrust" title="人人贷WE理财已经成为中国互联网信用评价中心网络诚信联盟成员，并且完成企业信用评级 " href="http://www.itrust.org.cn/Home/Index/itrust_certifi?wm=1012817080" target="_blank"></a>
        <a class="ui-footer-verification-item fn-left norton" title="人人贷WE理财已引入VeriSign SSL加密技术，您的隐私及个人资料安全已受最高级别的保护。" href="https://trustsealinfo.websecurity.norton.com/splash?form_file=fdf/splash.fdf&dn=www.we.com&lang=zh_cn" target="_blank" ></a>
        <!-- <a class="ui-footer-verification-item fn-left police" title="人人贷已经完成在公安机关的信息备案，您可了解网站相关备案信息。" href="http://gawa.bjchy.gov.cn/websearch/" target="_blank"></a>
        <a class="ui-footer-verification-item fn-left gongshang" title="人人贷已经完成在北京市工商局网站备案，您可了解网站相关备案信息。" href="http://www.hd315.gov.cn/beian/view.asp?bianhao=010202013052900002" target="_blank"></a> -->
      </div>
    </div>
  </div>
</div>
<ul class="fixed-download-goBack">
  <li class="fixed-calculator">
    <a class="fixed-icon" target="_blank" href="/lend/calculator.action?prodType=Loanplan"></a>
  </li>
  <li class="fixed-service">
    <a class="fixed-icon" href="javascript:" onclick="online_service();"></a>
  </li>
  <li class="fixed-download">
    <a class="fixed-icon" href="javascript:"></a>
    <div class="download-app-wrap-opacity">
       <div class="fixed-arrow"></div>
    </div>
    <div class="download-app-wrap">
      <h2>人人贷WE理财APP下载</h2>
      <div class="fn-clear">
        <div class="ewm-big">
          <img src="/TP/one/Public/images/.com/fund/info/tag000001/data:image/png;base64," alt="二维码" />
        </div>
        <div class="download-btns">
          <a target="_blank" class="ios" href="http://itunes.apple.com/us/app/id883561142?mt=8"><img src="/TP/one/Public/images/.com/fund/info/tag000001/data:image/png;base64," alt="ios下载" /></a>
          <a target="_blank" href="http://www.we.com/event/download.action?type=apk"><img src="/TP/one/Public/images/.com/fund/info/tag000001/data:image/png;base64," alt="android下载" /></a>
        </div>
      </div>      
    </div>
  </li>
  <li class="fixed-goTop">
    <a class="fixed-icon" href="javascript:"></a>
  </li>
</ul>
<div class="fn-hide">
  <div id="weixin-content" class="p20 text-center">
    <img src="/TP/one/Public/images/weixin_l.jpg" style="width:250px" />
    <p>微信扫一扫，人人贷We理财里动态全知道！</p>
  </div>
</div>
<script type='text/javascript'>
function online_service() {
    
    window.open('http://chat.looyuoms.com/chat/chat/p.do?c=20000293&f=10042100&g=10048378&site=5372&r=%23params%3A姓名%2C游客', '_blank', 'height=544, width=644,toolbar=no,scrollbars=no,menubar=no,status=no');
    
}
</script>
            
<script id="dialog-message" type="text/x-handlebars-template">
<div class="ui-message-content j-result-dialog">
    <div class="h50"></div>
  {{# if info }}
  <div class="j-dialog-result-icon j-dialog-icon-info" title="提示"></div>
  {{/ if }}
  {{# if success }}
    <div class="j-dialog-result-icon j-dialog-icon-success" title="成功"></div>
  {{/ if }}
  {{# if warning }}
    <div class="j-dialog-result-icon j-dialog-icon-warn" title="警告"></div>
  {{/ if }}
  {{# if error }}
    <div class="j-dialog-result-icon j-dialog-icon-error" title="错误"></div>
  {{/ if }}
  <div class="fn-clear">
    
      
      
      
      
      
      
      
      
      
      
      
      
    
    <div class="j-dialog-message ">
      {{# if title }}
      <h3>{{ title }}</h3>
      {{/ if }}
      <p class="text-big j-result-text">{{{ message }}}</p>
      {{# if link }}
      <p class="text-center">{{ linkText }}<a href="{{ link }}">{{ linkTextEnd }}</a>。</p>
      {{/ if }}
      {{# if fund}}
      <p class="text-center">{{fundMsg}}</p>
      {{/ if}}
    </div>
  </div>
  <div class="ui-message-operation text-center">
    {{# if fund}}
    <a class="ui-message-close-button j-btn j-btn-orange  j-dialog-btn-small" style="width: 90px;" {{# if button }}href="{{ button.link }}"{{/ if }}>{{# if button }}{{ button.text }}{{ else }}关 闭{{/ if }}</a>
    <a class="j-btn j-btn-cancel j-dialog-btn-small" style="width: 90px;" id="lookDetailBtn" href="{{fundUrl}}">查看详情</a>
    {{else}}
      <a class="ui-button   ui-message-close-button j-btn j-btn-orange j-dialog-small" {{# if button }}href="{{ button.link }}"{{/ if }}>{{# if button }}{{ button.text }}{{ else }}关 闭{{/ if }}</a>
    {{/if}}
  </div>
</div>
</script>
        
        <!--body里面的js-->
        
    	 <script id="fundlist-index-data" type="text/x-json">{"data":{"pageCount":82,"pageIndex":1,"resultList":[{"abbrev":"鹏华弘盛C","allotRate":[["0<=申购金额<1000万","1.50%","0.60%"],["1000万<=申购金额","1000.00元","1000.00元"]],"confirmPace":"T+1","fee":"0.60","fundChannel":"YM","fundCode":"001380","fundStatus":"0","fundType":"3","maxIndividualAllotAmount":"9999999999.00","maxIndividualSubscribeAmount":"0","minIndividualAllotAmount":"1000.00","minIndividualSubscribeAmount":"0","minSubscript":"1000.00","nav":"1.4826","onSale":"1","originFee":"1.50","rateGrowth":"0.06","rateHalfYear":"44.08","rateMonth":"0.55","rateSeason":"1.48","rateYear":"47.67","redeemRate":[["0天<持有天数<=29天","0.50%"],["29天<持有天数","0.00%"]],"refundPace":"T+3","riskLevel":"3","saleRate":"4.0","subscribeRate":[["--","0.00%","0.00%"]],"thisYear":"43.66"},{"abbrev":"兴业聚利","allotRate":[["0<=申购金额<50万","1.50%","0.60%"],["50万<=申购金额<100万","1.00%","0.60%"],["100万<=申购金额<500万","0.60%","0.60%"],["500万<=申购金额","1000.00元","1000.00元"]],"confirmPace":"T+1","fee":"0.60","fundChannel":"YM","fundCode":"001272","fundStatus":"0","fundType":"3","maxIndividualAllotAmount":"9999999999.00","maxIndividualSubscribeAmount":"0","minIndividualAllotAmount":"1000.00","minIndividualSubscribeAmount":"0","minSubscript":"1000.00","nav":"1.3790","onSale":"1","originFee":"1.50","rateGrowth":"0.58","rateHalfYear":"24.23","rateMonth":"1.03","rateSeason":"4.63","rateYear":"37.35","redeemRate":[["0天<持有天数<=6天","1.50%"],["6天<持有天数<=29天","0.75%"],["29天<持有天数<=364天","0.50%"],["364天<持有天数","0.00%"]],"refundPace":"T+3","riskLevel":"3","saleRate":"4.0","subscribeRate":[["0<=认购金额<50万","1.20%","0.60%"],["50万<=认购金额<100万","0.80%","0.60%"],["100万<=认购金额<500万","0.50%","0.50%"],["500万<=认购金额","1000.00元","1000.00元"]],"thisYear":"20.02"},{"abbrev":"华泰柏瑞消费成长","allotRate":[["0<=申购金额<100万","1.50%","0.60%"],["100万<=申购金额<300万","1.00%","0.60%"],["300万<=申购金额<500万","0.80%","0.60%"],["500万<=申购金额","1000.00元","1000.00元"]],"confirmPace":"T+1","fee":"0.60","fundChannel":"YM","fundCode":"001069","fundStatus":"0","fundType":"3","maxIndividualAllotAmount":"9999999999.00","maxIndividualSubscribeAmount":"0","minIndividualAllotAmount":"1000.00","minIndividualSubscribeAmount":"0","minSubscript":"1000.00","nav":"1.2000","onSale":"1","originFee":"1.50","rateGrowth":"0.25","rateHalfYear":"-13.17","rateMonth":"4.53","rateSeason":"21.95","rateYear":"19.88","redeemRate":[["0天<持有天数<=6天","1.50%"],["6天<持有天数<=29天","0.75%"],["29天<持有天数<=364天","0.50%"],["364天<持有天数<=729天","0.25%"],["729天<持有天数","0.00%"]],"refundPace":"T+3","riskLevel":"3","saleRate":"4.0","subscribeRate":[["0<=认购金额<100万","1.20%","0.60%"],["100万<=认购金额<300万","0.80%","0.60%"],["300万<=认购金额<500万","0.60%","0.60%"],["500万<=认购金额","1000.00元","1000.00元"]],"thisYear":"-10.98"},{"abbrev":"国泰结构转型A","allotRate":[["0<=申购金额<100万","0.80%","0.60%"],["100万<=申购金额<300万","0.50%","0.50%"],["300万<=申购金额<500万","0.30%","0.30%"],["500万<=申购金额","1000.00元","1000.00元"]],"confirmPace":"T+1","fee":"0.60","fundChannel":"YM","fundCode":"000512","fundStatus":"0","fundType":"3","maxIndividualAllotAmount":"9999999999.00","maxIndividualSubscribeAmount":"0","minIndividualAllotAmount":"100.00","minIndividualSubscribeAmount":"0","minSubscript":"100.00","nav":"1.6690","onSale":"1","originFee":"0.80","rateGrowth":"0.12","rateHalfYear":"0.97","rateMonth":"1.21","rateSeason":"1.21","rateYear":"16.71","redeemRate":[["0天<持有天数<=6天","1.50%"],["6天<持有天数<=29天","0.75%"],["29天<持有天数<=179天","0.50%"],["179天<持有天数<=364天","0.10%"],["364天<持有天数<=729天","0.05%"],["729天<持有天数","0.00%"]],"refundPace":"T+3","riskLevel":"3","saleRate":"7.5","subscribeRate":[["0<=认购金额<100万","0.60%","0.60%"],["100万<=认购金额<300万","0.40%","0.40%"],["300万<=认购金额<500万","0.20%","0.20%"],["500万<=认购金额","1000.00元","1000.00元"]],"thisYear":"0.48"},{"abbrev":"浦银安盛盛世精选A","allotRate":[["0<=申购金额<100万","1.50%","0.60%"],["100万<=申购金额<300万","1.00%","0.60%"],["300万<=申购金额<500万","0.30%","0.30%"],["500万<=申购金额","1000.00元","1000.00元"]],"confirmPace":"T+1","fee":"0.60","fundChannel":"YM","fundCode":"519127","fundStatus":"0","fundType":"3","maxIndividualAllotAmount":"9999999999.00","maxIndividualSubscribeAmount":"0","minIndividualAllotAmount":"1000.00","minIndividualSubscribeAmount":"0","minSubscript":"1000.00","nav":"1.3380","onSale":"1","originFee":"1.50","rateGrowth":"0.22","rateHalfYear":"5.02","rateMonth":"0.98","rateSeason":"4.86","rateYear":"16.35","redeemRate":[["0天<持有天数<=6天","1.50%"],["6天<持有天数<=29天","0.75%"],["29天<持有天数<=364天","0.50%"],["364天<持有天数<=729天","0.25%"],["729天<持有天数","0.00%"]],"refundPace":"T+3","riskLevel":"3","saleRate":"4.0","subscribeRate":[["0<=认购金额<100万","1.20%","0.60%"],["100万<=认购金额<300万","0.80%","0.60%"],["300万<=认购金额<500万","0.20%","0.20%"],["500万<=认购金额","1000.00元","1000.00元"]],"thisYear":"4.69"},{"abbrev":"鹏华弘信A","allotRate":[["0<=申购金额<100万","1.50%","0.60%"],["100万<=申购金额<500万","1.00%","0.60%"],["500万<=申购金额<1000万","0.30%","0.30%"],["1000万<=申购金额","1000.00元","1000.00元"]],"confirmPace":"T+1","fee":"0.60","fundChannel":"YM","fundCode":"001331","fundStatus":"0","fundType":"3","maxIndividualAllotAmount":"9999999999.00","maxIndividualSubscribeAmount":"0","minIndividualAllotAmount":"1000.00","minIndividualSubscribeAmount":"0","minSubscript":"1000.00","nav":"1.1380","onSale":"1","originFee":"1.50","rateGrowth":"10.50","rateHalfYear":"12.67","rateMonth":"11.07","rateSeason":"11.46","rateYear":"14.26","redeemRate":[["0天<持有天数<=6天","1.50%"],["6天<持有天数<=29天","0.75%"],["29天<持有天数<=179天","0.50%"],["179天<持有天数","0.00%"]],"refundPace":"T+3","riskLevel":"3","saleRate":"4.0","subscribeRate":[["0<=认购金额<100万","1.20%","0.60%"],["100万<=认购金额<500万","0.70%","0.60%"],["500万<=认购金额<1000万","0.20%","0.20%"],["1000万<=认购金额","1000.00元","1000.00元"]],"thisYear":"11.68"},{"abbrev":"前海开源事件驱动A","allotRate":[["0<=申购金额<50万","1.50%","0.60%"],["50万<=申购金额<250万","1.20%","0.60%"],["250万<=申购金额<500万","0.80%","0.60%"],["500万<=申购金额","1000.00元","1000.00元"]],"confirmPace":"T+1","fee":"0.60","fundChannel":"YM","fundCode":"000423","fundStatus":"0","fundType":"3","maxIndividualAllotAmount":"9999999999.00","maxIndividualSubscribeAmount":"0","minIndividualAllotAmount":"1000.00","minIndividualSubscribeAmount":"0","minSubscript":"1000.00","nav":"1.1790","onSale":"1","originFee":"1.50","rateGrowth":"0.51","rateHalfYear":"9.88","rateMonth":"6.60","rateSeason":"7.18","rateYear":"11.65","redeemRate":[["0天<持有天数<=6天","1.50%"],["6天<持有天数<=29天","0.75%"],["29天<持有天数<=365天","0.50%"],["365天<持有天数<=730天","0.25%"],["730天<持有天数","0.00%"]],"refundPace":"T+3","riskLevel":"3","saleRate":"4.0","subscribeRate":[["0<=认购金额<50万","1.20%","0.60%"],["50万<=认购金额<250万","1.00%","0.60%"],["250万<=认购金额<500万","0.60%","0.60%"],["500万<=认购金额","1000.00元","1000.00元"]],"thisYear":"9.57"},{"abbrev":"兴业聚优","allotRate":[["0<=申购金额<50万","1.50%","0.60%"],["50万<=申购金额<100万","1.00%","0.60%"],["100万<=申购金额<500万","0.60%","0.60%"],["500万<=申购金额","1000.00元","1000.00元"]],"confirmPace":"T+1","fee":"0.60","fundChannel":"YM","fundCode":"001321","fundStatus":"0","fundType":"3","maxIndividualAllotAmount":"9999999999.00","maxIndividualSubscribeAmount":"0","minIndividualAllotAmount":"1000.00","minIndividualSubscribeAmount":"0","minSubscript":"1000.00","nav":"1.1200","onSale":"1","originFee":"1.50","rateGrowth":"0.18","rateHalfYear":"5.96","rateMonth":"0.54","rateSeason":"0.36","rateYear":"11.55","redeemRate":[["0天<持有天数<=6天","1.50%"],["6天<持有天数<=29天","0.75%"],["29天<持有天数<=364天","0.50%"],["364天<持有天数","0.00%"]],"refundPace":"T+3","riskLevel":"3","saleRate":"4.0","subscribeRate":[["0<=认购金额<50万","1.20%","0.60%"],["50万<=认购金额<100万","0.80%","0.60%"],["100万<=认购金额<500万","0.50%","0.50%"],["500万<=认购金额","1000.00元","1000.00元"]],"thisYear":"5.16"},{"abbrev":"长盛电子信息主题","allotRate":[["0<=申购金额<50万","1.50%","0.60%"],["50万<=申购金额<200万","1.00%","0.60%"],["200万<=申购金额<500万","0.50%","0.50%"],["500万<=申购金额","1000.00元","1000.00元"]],"confirmPace":"T+1","fee":"0.60","fundChannel":"YM","fundCode":"000063","fundStatus":"0","fundType":"3","maxIndividualAllotAmount":"9999999999.00","maxIndividualSubscribeAmount":"0","minIndividualAllotAmount":"1000.00","minIndividualSubscribeAmount":"0","minSubscript":"1000.00","nav":"1.9820","onSale":"1","originFee":"1.50","rateGrowth":"0.30","rateHalfYear":"-19.23","rateMonth":"2.80","rateSeason":"10.48","rateYear":"11.04","redeemRate":[["0天<持有天数<=364天","0.50%"],["364天<持有天数<=729天","0.25%"],["729天<持有天数","0.00%"]],"refundPace":"T+3","riskLevel":"3","saleRate":"4.0","subscribeRate":[["--","0.00%","0.00%"]],"thisYear":"-21.69"},{"abbrev":"广发趋势优选","allotRate":[["0<=申购金额<100万","1.50%","0.60%"],["100万<=申购金额<500万","0.90%","0.60%"],["500万<=申购金额<1000万","0.30%","0.30%"],["1000万<=申购金额","1000.00元","1000.00元"]],"confirmPace":"T+1","fee":"0.60","fundChannel":"YM","fundCode":"000215","fundStatus":"0","fundType":"3","maxIndividualAllotAmount":"9999999999.00","maxIndividualSubscribeAmount":"0","minIndividualAllotAmount":"10.00","minIndividualSubscribeAmount":"0","minSubscript":"10.00","nav":"1.4160","onSale":"1","originFee":"1.50","rateGrowth":"0.07","rateHalfYear":"2.02","rateMonth":"0.71","rateSeason":"2.31","rateYear":"10.80","redeemRate":[["0天<持有天数<=364天","0.50%"],["364天<持有天数<=729天","0.30%"],["729天<持有天数","0.00%"]],"refundPace":"T+3","riskLevel":"3","saleRate":"4.0","subscribeRate":[["0<=认购金额<100万","1.20%","0.60%"],["100万<=认购金额<500万","0.80%","0.60%"],["500万<=认购金额<1000万","0.20%","0.20%"],["1000万<=认购金额","1000.00元","1000.00元"]],"thisYear":"1.36"}],"totalPage":82},"message":"success","status":0}</script>
        
    
        
        
        
        
        <!-- we理财百度统计 -->
        <script>
        var _hmt = _hmt || [];
        (function() {
          var hm = document.createElement("script");
          hm.src = "//hm.baidu.com/hm.js?16f9bb97b83369e62ee1386631124bb1";
          var s = document.getElementsByTagName("script")[0];
          s.parentNode.insertBefore(hm, s);
        })();
        </script>
        
    <script type="text/javascript" src="/TP/one/Public/js/require_2f19223.js"></script><script type="text/javascript" src="/TP/one/Public/js/common_widget_78d2def.js"></script><script type="text/javascript" src="/TP/one/Public/js/list_e6618da.js"></script><script type="text/javascript">;require(["common:widget/second-header/second-header"], function(header){
        header.init();
    });
;require(["common:widget/footer/footer"], function(){});
;require(['fund:page/list/list'], function(list){
               list.init();
            });
;require(['common:widget/ui/messageModule/messageModule'], function(shit){
                shit.init();
            });
;require(["common:widget/ui/utmsource/utmsource"], function(){});
;require(['common:widget/ui/service-change-dialog/service-change-dialog'], function( obj ){
                var isShow = '' === 'true';
                if( isShow ){
                    obj.show();
                }
            });
;weLogger.perf.bodyEnd( );
            if( typeof document.addEventListener !== 'function' ){
                weLogger.perf.domReady();
            }
</script></body>
    
    <!-- THX FOR FIS -->
</html>